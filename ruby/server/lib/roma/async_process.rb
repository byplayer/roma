...
module Roma
  ...
  module AsyncProcess
    ...
    def asyncev_start_dumpfile_process(args)
      ...
      key, path, cmd = args
      path = Config::STORAGE_DUMP_PATH + '/' + path
      Thread.new{
        begin
          except_vnh = {}
    ...
    def delete_to_end_of_dump(key)
      con = Config::HANDLER_CLASS::con_pool.get_connection(@stats.ap_str)
      con.write("delete #{key}\eroma\r\n")
      res = con.gets
      res.chomp! if res
      Config::HANDLER_CLASS::con_pool.return_connection(@stats.ap_str, con)
      res
    end
    ...
    def req_push_a_vnode(vn, src_nid, is_primary)
      ...
      Config::HANDLER_CLASS::con_pool.return_connection(src_nid, con)
      # waiting for pushv
      count = 0
...
#      @log.info("#{__method__}:start")
      ...
      t = Time.now.to_i - Config::STORAGE_DELMARK_EXPTIME
      count = 0
      @storages.each_pair{|hname,st|
